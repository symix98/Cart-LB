    ...
    async sendMail(req, res, next) {
        try {
            ...
            let mailTransporter = nodemailer.createTransport({
                service: 'gmail',
                auth: {
                    user: 'smartmartpostmaster@gmail.com',
                    pass: 'ygrhgfyacujulzma',
                }
            });

            let mailDetails = {
                from: 'smartmartpostmaster@gmail.com',
                to: 'smartmartsales1@gmail.com',
                subject: "Order# " + oid,
                // text: req.body,
                html:
                    `
                ...
                <h3 style="color: #fff;">Net Price: ${netPrice}</h3>
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            <td style="color:white;text-align: center;">${products.map(product => `<p>${product.pdesc}</p>`)}</td>
                            <td style="color:white;text-align: center;">${items.map(item => `<p>${item.olqtty}</p>`)}</td>
                            <td style="color:white;text-align: center;">${items.map(item => `<p>${item.olprice}</p>`)}</td>
                        </tr>
                    </tbody>
